...
exports.validateVotingParams = function(req, res, next) {
    const userVote = parseInt(req.body.vote);
    if (isNaN(userVote)) {
        res.status(400).send(`${req.body.vote} is definitely not a number\nTry vote again.`);
        const numberErr = new Error(`${req.body.vote} is definitely not a number\nTry vote again.`);
        next(numberErr);
    } else if (userVote > 5) {
        const voteError = new Error(`You voted ${userVote}, Max vote is 5`);
        next(voteError);
    } else {
        next();
    }
...
exports.updateUserTeamVote = function(req, res) {
    ...
    Team.findById(req.params.teamId, (err, team) => {
        if (team) {
            let tasksArr = [];
            tasksArr.push(User.updateUserVote.bind(User, req.user._id.toString(), req.params.teamId, userVote));
            tasksArr.push(team.updateTeamVote.bind(team));
            async.waterfall(tasksArr, (err, teamId) => {
                if (err) {
                    console.error("Error on user vote");
                    res.status(500).send("Error on vote");
                } else {
                    res.send({teamId: teamId, vote: userVote});
                }
            });
        } else {
            console.error(`There is no such team with ID of ${teamId}`);
            res.status(404).send(`Could not find a team with ID of ${teamId}`)
        }
    });
};
